...
if (!parentPort) {
    throw new Error("saxonWorker must be run as a worker thread");
}
...
parentPort.on("message", async (job: TransformJob) => {
    try {
        ...
        parentPort!.postMessage({ success: true, result: transformResult });
    } catch (error: any) {
